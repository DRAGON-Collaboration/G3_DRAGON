...
C
      COMMON  /MITRAY4/  ENERGY, VEL, PMASS, Q0
...
C****
C****
      IF(IFLAG .NE. 0)WRITE(6,105) II, IMAP,IR, NXLO, NXHI, NZLO, NZHI
C****
C**** IR=2
C****
      IFLAG = 0
...
C****
      ...
      DELS = DATA( 45,II )
      RCA  = DATA( 47,II )
      S2   = DATA( 51,II ) / RB    + RCA/2.D0
      S3   = DATA( 52,II ) / RB**2
      S4   = DATA( 53,II ) / RB**3 + RCA**3/8.D0
      S5   = DATA( 54,II ) / RB**4
      S6   = DATA( 55,II ) / RB**5 + RCA**5/16.D0
      S7   = DATA( 56,II ) / RB**6
      S8   = DATA( 57,II ) / RB**7 + RCA**7/25.6D0
C****
C**** CHECK IF WE HAVE A FLAT BOUNDARY
C****       NSRF=0 FLAT
C****           =1 CURVED
...
C****
      ...
      DELS = DATA( 46,II )
      RCA  = DATA( 48,II )
      S2   = DATA( 58,II ) / RB    + RCA/2.D0
      S3   = DATA( 59,II ) / RB**2
      S4   = DATA( 60,II ) / RB**3 + RCA**3/8.D0
      S5   = DATA( 61,II ) / RB**4
      S6   = DATA( 62,II ) / RB**5 + RCA**5/16.D0
      S7   = DATA( 63,II ) / RB**6
      S8   = DATA( 64,II ) / RB**7 + RCA**7/25.6D0
C****
C**** CHECK IF WE HAVE A FLAT BOUNDARY
C****       NSRF=0 FLAT
C****           =1 CURVED
...
C****
      ...
      X = (I-IX)*DG
      Z = (J-IZ)*DG
      CALL MITRAY_BDPP( B0,Z,X)
      BZMAP(I, J, IR, IMAP ) = B0/BF
    2 CONTINUE
C****
C****
    4 WRITE(6,106) IMAP, II, MTYP, IR, NXLO, NXHI, NZLO, NZHI
    1 CONTINUE
C****
C**** PRINT MAPS
C***
      ...
      IF( NP .LE. 100 ) THEN
      DO 10 IR=1,2
      IF( IR .EQ. 1 ) WRITE(6,100) IMAP
      IF( IR .EQ. 2 ) WRITE(6,101) IMAP
      NXLO = NXLMT(IMAP,IR,1)
      NXHI = NXLMT(IMAP,IR,2)
      NZLO = NZLMT(IMAP,IR,1)
      NZHI = NZLMT(IMAP,IR,2)
      ...
      J1 = I
      J2 = I+14
      IF( J1 .GT. NXLO ) WRITE(6,102)
      IF( J2 .GT. NXHI ) J2 = NXHI
      WRITE(6,103) (J, J=J1,J2)
      WRITE(6,107) ( (J-51)*DG, J=J1,J2)
      WRITE(6,108)
      DO 12 KSK=NZLO,NZHI
      L = KSK
      WRITE(6,104)L, (L-IZ)*DG, (BZMAP(J, L, IR, IMAP), J=J1,J2 )
   12 CONTINUE
   11 CONTINUE
   10 CONTINUE
      END IF
