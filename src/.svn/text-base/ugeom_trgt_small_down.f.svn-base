C     Small Pumping tubes
      SUBROUTINE ugeo_trgt_small_down
      IMPLICIT NONE
      include 'uggeom.inc'
...
C.
      ...
      CALL gspos('ATC1',1,'EX9G',0.0,-1.2885+0.7+2.5,8.93048077,
     +   0,'MANY') 
C.
C. **** Do -y border
C.
...
C.
      ...
      shape(2) = 2.5
      shape(3) = 0.025
      CALL gsvolu ('ATC2', 'BOX ', 5, shape, 3, ivolu)
      CALL gsatt('ATC2','SEEN',1)
      CALL gspos('ATC2',1,'EX9G',0.0,-1.2885-0.7-2.5,8.93048077,
     +   0,'MANY')
...
C.      
      ...
      CALL gspos('ATC3',1,'EX9G',.465+2.5,0.0,8.93048077,0,'MANY') 
C.
C. **** Do -x border
C.
C.
C.
      ...
      shape(2) = 2.5
      shape(3) = 0.025
      CALL gsvolu ('ATC4', 'BOX ', 5, shape, 3, ivolu)
      CALL gsatt('ATC4','SEEN',1)
      CALL gspos('ATC4',1,'EX9G',-0.465-2.5,0.0,8.93048077,0,'MANY')
C.
