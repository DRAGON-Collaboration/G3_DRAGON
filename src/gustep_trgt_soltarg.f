...
c     c mt        If(chname_nlevel .eq.'EAPG' .and.ipart .eq. irecoil)
     &        ngascell = ngascell +1
      Endif
C     .
C
...
C     .
C     JS changed to CMBG from CELL.  No CELL in solid target config.
            If(chname_nlevel.eq.'CMBG')then
...
C     .
C     .       JS changed to CMBG from CELL, no CELL in solid targ config.
            If(chname_nlevel.eq.'CMBG')then
...
C     .
      Endif
C     .
C     .    JS changed to EX6G from TEND, TEND does not work properly in new tubes
      If(inwvol.eq.1 .and. chname_nlevel.eq.'EX6G')then
C     .
...
C     .
      If(istop.ne.0)then
C     write(6,*)' Whats stopping me??? (in trgt)'
C     write(6,*)' istop: ',istop,' Volume: ',chname_nlevel,"  ",sleng,
C     &            ' ipart: ',ipart
      Endif
      ngkine = 0
C     .
